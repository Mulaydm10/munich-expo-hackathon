...
| 2026-09-05 | `COMPETITION.md` | devin-ai-integration[bot] (design node, PR `design/…-ticket-open-action`) | One line: the participation-mode `TODO(Mulaydm10)` now points at `Q-0008` instead of restating the decision inline. No event fact changed — mode is still unset, no ticket is still held. Moved because a to-do written into a facts table reads as settled, and this one can lose the submission |
